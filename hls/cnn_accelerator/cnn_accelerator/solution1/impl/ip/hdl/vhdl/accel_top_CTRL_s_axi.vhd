...
entity accel_top_CTRL_s_axi is
generic (
...
end entity accel_top_CTRL_s_axi;
...
architecture behave of accel_top_CTRL_s_axi is
    ...
    constant ADDR_OUTPUT_DATA_DATA_0  : INTEGER := 16#34#;
    constant ADDR_OUTPUT_DATA_DATA_1  : INTEGER := 16#38#;
    constant ADDR_OUTPUT_DATA_CTRL    : INTEGER := 16#3c#;
    constant ADDR_KERNEL_SIZE_DATA_0  : INTEGER := 16#40#;
    constant ADDR_KERNEL_SIZE_CTRL    : INTEGER := 16#44#;
    constant ADDR_IN_CHANNELS_DATA_0  : INTEGER := 16#48#;
    constant ADDR_IN_CHANNELS_CTRL    : INTEGER := 16#4c#;
    constant ADDR_OUT_CHANNELS_DATA_0 : INTEGER := 16#50#;
    constant ADDR_OUT_CHANNELS_CTRL   : INTEGER := 16#54#;
    constant ADDR_IN_HEIGHT_DATA_0    : INTEGER := 16#58#;
    constant ADDR_IN_HEIGHT_CTRL      : INTEGER := 16#5c#;
    constant ADDR_IN_WIDTH_DATA_0     : INTEGER := 16#60#;
    constant ADDR_IN_WIDTH_CTRL       : INTEGER := 16#64#;
    constant ADDR_STRIDE_DATA_0       : INTEGER := 16#68#;
    constant ADDR_STRIDE_CTRL         : INTEGER := 16#6c#;
    constant ADDR_PADDING_DATA_0      : INTEGER := 16#70#;
    constant ADDR_PADDING_CTRL        : INTEGER := 16#74#;
    constant ADDR_RELU_ENABLE_DATA_0  : INTEGER := 16#78#;
    constant ADDR_RELU_ENABLE_CTRL    : INTEGER := 16#7c#;
    constant ADDR_POOL_ENABLE_DATA_0  : INTEGER := 16#80#;
    constant ADDR_POOL_ENABLE_CTRL    : INTEGER := 16#84#;
    ...
    signal int_relu_enable     : UNSIGNED(31 downto 0) := (others => '0');
    signal int_pool_enable     : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_RELU_ENABLE_DATA_0 =>
                        rdata_data <= RESIZE(int_relu_enable(31 downto 0), 32);
                    when ADDR_POOL_ENABLE_DATA_0 =>
                        rdata_data <= RESIZE(int_pool_enable(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    relu_enable          <= STD_LOGIC_VECTOR(int_relu_enable);
    pool_enable          <= STD_LOGIC_VECTOR(int_pool_enable);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_RELU_ENABLE_DATA_0) then
                    int_relu_enable(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_relu_enable(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_POOL_ENABLE_DATA_0) then
                    int_pool_enable(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_pool_enable(31 downto 0));
                end if;
            end if;
        end if;
    end process;
